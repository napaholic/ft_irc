...
void    Channel::addClient(const std::string &nick)
{
    if (!isClient(nick) && !isBanned(nick))
        nicks.insert(std::pair<std::string, unsigned char>(nick, opt_o));
    else
...
const unsigned char Channel::get_permissions(const std::string &nick)
{
    if (!isClient(nick))
        return (NULL);
    for (std::map<std::string, unsigned char>::iterator it = __nicks.begin();
...
void	Channel::setPermissions(const std::string &nick, const unsigned char perm)
{
...
void	Channel::change_nick(const std::string &old_nick, const std::string &nick)
{
    ...
    {
        nicks.insert(std::pair<std::string, unsigned char>(nick, get_permissions(old_nick)));
